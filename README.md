...
| --- | --- | --- |
| `BLE_I3_TITLEBAR` | every window keeps its border but loses the bar with the window name in it — 3px wide, 4px on the focused window, in both desktops | the bar you grab to move a floating window. `$mod`+left-drag still moves it, and the border is still the mouse resize handle |
...
that is what keeps the tiling desktop's own 3px border *and* drops its
...
The same script's `00-tiling-border.local` (the tiled border width) sorts
...
| `magic-trackpad` | Ubuntu repos (`libinput-tools`, `xdotool`) + the files in `magic-trackpad/` — classic scrolling, two-finger back/forward in Firefox and three-finger swipes to change browser tab, [below](#-magic-trackpad) |
...
- **Two-finger swipe → back / forward in Firefox.** Chrome and Brave do this on
  their own. Firefox on X only does it when it reads the trackpad through
  XInput2, which is still opt-in, so the module adds
  `export MOZ_USE_XINPUT2=1` to `~/.xsessionrc`.
- **Three-finger swipe → browser tab.** In Firefox, Chrome or Brave, swipe left
  for the tab on the left and right for the tab on the right (no inverted
  direction). In any other window the swipe does nothing. X gives i3 no
  gestures, so `ble-trackpad-gestures` (in `~/.local/bin`) reads them from
...
reboot): X reads `xorg.conf.d` and `~/.xsessionrc` only when it starts, and a
new group only reaches a new login.
